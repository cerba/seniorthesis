...
# Define Histogram
gluplot = r.TH1D('Gluon Transverse Momentum','g,p_T',100,0,100)
bplot = r.TH1D('Bottom Transverse Momentum','b;p_t',100,0,100)
abplot = r.TH1D('Anti-Bottom Transverse Momentum','#bar{b};p_T',100,0,100)
tplot = r.TH1D('Top Transverse Momentum','t;p_t',100,0,100)
atplot = r.TH1D('Anti-Top Transverse Momentum','#bar{t};p_T',100,0,100)
...
for j in range(tree.GetEntries()) :
    ...
    qw = next((i for i in index if tree.genMotherIndex[i] == 24 and tree.genPdgId[i] == 1 or tree.genPdgId[i] == 2 or tree.genPdgId[i] == 3 or tree.genPdgId[i] == 4 or tree.genPdgId[i] == 5 or tree.genPdgId[i] == 6), None)          
    aqw = next((i for i in index if tree.genMotherIndex[i] == -24 and tree.genPdgId[i] == -1 or tree.genPdgId[i] == -2 or tree.genPdgId[i] == -3 or tree.genPdgId[i] == -4 or tree.genPdgId[i] == -5 or tree.genPdgId[i] == -6), None)          
...
for hist in [gluplot, bplot, abplot, tplot, atplot, qwplot, aqwplot, glubAngl, gluBbarAngl, glutAngl, gluTbarAngl, gluQwAngl, gluAqwAngl] :
    hist.Draw()
